...
class GameScene extends eui.Component {
    private panelArray = [[null, null, null], [null, null, null], [null, null, null]];
    private shapeArr = [[null, null, null], [null, null, null], [null, null, null]];
    private readyCircle = [null, null, null];
    ...
    private initReadyCircle() {
        for (var i = 0; i < 3; i++) {
            this.createReadyCircle(i);
        }
    }

    private createTestCircle(i) {
        var circle = new Circle();
        circle.readyId = i;
    ...
    private touchCircle:Circle = null;
    ...
    private getTouchCircle(x, y) {
        for (var k  in this.readyCircle) {
            var circle:Circle = this.readyCircle[k];
    ...
    private onTouchEnd(touch:egret.TouchEvent) {
        ...
        for (var row = 0; row < 3; row++) {
            for (var column = 0; column < 3; column++) {
                var item:eui.Rect = this.panelArray[row][column];
                var b = item.hitTestPoint(x, y);
                if (b) {
                    //console.log(row + "----" + column + "***" + b);
                    var circle:Circle = this.shapeArr[row][column];
    ...
    private isGameOver() {
        for (var row = 0; row < 3; row++) {
            for (var column = 0; column < 3; column++) {
                var item:Circle = this.shapeArr[row][column];
    ...
    // 获取相同行
    private getSameRow(row, column, color, cleanList) {
        var up = row - 1;
        var down = row + 1;
        var left = column - 1;
        var right = column + 1;
    ...
    // 获取相同列
    private getSameColumn(row, column, color, cleanList) {
        var up = row - 1;
        var down = row + 1;
        var left = column - 1;
        var right = column + 1;
        ...
        // 上边
        while (up >= 0) {
            var upCell:Circle = this.shapeArr[up][column];
        ...
        // 下边
        while (down < 3) {
            var downCell:Circle = this.shapeArr[down][column];
    ...
    // 获取斜着的
    private getSameSlope1(row, column, color, cleanList) {
        var up = row - 1;
        var down = row + 1;
        var left = column - 1;
        var right = column + 1;
        ...
        // 右上
        while (up >= 0 && right < 3) {
            var upRightCell:Circle = this.shapeArr[up][right];
        ...
        // 左下
        while (down < 3 && left >= 0) {
            var downLeftCell:Circle = this.shapeArr[down][left];
    ...
    private getSameSlope2(row, column, color, cleanList) {
        var up = row - 1;
        var down = row + 1;
        var left = column - 1;
        var right = column + 1;
        // 左上
        while (up >= 0 && left >= 0) {
            var upLeftCell:Circle = this.shapeArr[up][left];
        ...
        while (down < 3 && right < 3) {
            var downRightCell:Circle = this.shapeArr[down][right];
    ...
    private cleanCircle() {
        ...
        for (var row = 0; row < 3; row++) {
            for (var column = 0; column < 3; column++) {
                var curCircle:Circle = this.shapeArr[row][column];
                // 检测自身是否可以清除
                ...
                for (var i = 0; i < colorArr.length; i++) {
                    ...
                    if (cleanListRow.length >= 3 && this.isRepeatList(cleanList, cleanListRow) == false) {//去掉重复的
        ...
        for (var k in oneColorList) {
            var item:Circle = oneColorList[k];
    ...
    private createLaser(cleanList) {
        for (var k in cleanList) {
            var list = cleanList[k];
            var obj1:Circle = list[0]['cell'];
            var obj2 = list[1]['cell'];
            var index1 = list[0]['index'][0];
            var color = obj1.colorData[index1];

            var p1:egret.Point = obj1 .localToGlobal(obj1.width / 2, obj1.height / 2);
            var p2:egret.Point = obj2 .localToGlobal(obj2.width / 2, obj2.height / 2);
    ...
    private createBoom(circle:Circle, color) {
        var p1:egret.Point = circle .localToGlobal(circle.width / 2, circle.height / 2);
        var boom = new Boom();
    ...
    private  isRepeatList(totalList, list) {
        //数据结构:[ [{},{},{}],  [{},{},{}], ]
    ...
    // 判断两个arr里面的元素是否相同
    private isSammeArray(list1, list2) {
        for (var k in list1) {
            var item1 = list1[k];
